package com.mvest.m.company.action;
...
public class CompanyListAction extends ActionModel {
	...
	/*	if(isNotNull(orderby)) {
		...
		try {
			int total = CompanyDao.getInstace().getListCount(where);
			Vo vo = getNumbersForPaging(total, curPage, numPerPage, pagePerBlock);
			addJsonObject("vo", vo);
			
			List<Company> companies = CompanyDao.getInstace().getList(vo.getStartRecord(), vo.getEndRecord(), where, orderby);
			
			for(Company c : companies){
				String co_code = c.getCo_code();
				try{
					Hashtable priceMap = getPrice(co_code);
					c.setTo_price((String)priceMap.get("to_price"));
					c.setTo_yesterday((String)priceMap.get("to_yesterday"));
					c.setTo_previous_exe();
			...
			if(companies != null) addJsonArray("companies", companies);
